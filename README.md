...
文本加解密、二维码、JSON、Base64 和文本整理工具不会把输入内容、密码或处理历史写入本地存储，也不会发送到服务器。文本加密使用 Web Crypto 的 PBKDF2-HMAC-SHA-256 与 AES-256-GCM；二维码由随应用打包的本地生成器完成；JSON 格式化会保留超大整数字面量与重复键，Base64 按 UTF-8 字节处理多语言和 Emoji。上述计算可在断网状态下运行，但当前 H5 尚未配置 Service Worker，因此首次打开页面仍需要应用资源已经加载或由后续离线 App 包内置。Base64 只是一种编码格式，不提供加密保护。
