...
- 工具：追番助手、AES-GCM 文本加解密、文字/文案二维码生成与 PNG 导出
...
文本加解密和二维码工具不会把输入内容、密码或生成历史写入本地存储，也不会发送到服务器。文本加密使用 Web Crypto 的 PBKDF2-HMAC-SHA-256 与 AES-256-GCM；二维码由随应用打包的本地生成器完成。两项计算可在断网状态下运行，但当前 H5 尚未配置 Service Worker，因此首次打开页面仍需要应用资源已经加载或由后续离线 App 包内置。
