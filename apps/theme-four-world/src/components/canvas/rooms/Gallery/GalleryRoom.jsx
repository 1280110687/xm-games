...
const GalleryRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    const backTextureRaw = useTexture(canHover ? '/theme-four-experience/textures/gallery/tylkartki_painted.webp' : '/theme-four-experience/textures/gallery/tylkartki.webp');
    const overlayTextureRaw = useTexture(canHover ? '/theme-four-experience/textures/gallery/przyciskdotylukartki_painted.webp' : '/theme-four-experience/textures/gallery/przyciskdotylukartki.webp');
    ...
    const allLogos = useMemo(() => {
        ...
        return names.map(name => {
            if (!canHover) return `/theme-four-experience/textures/gallery/${name}.webp`;
            if (name === 'csslogo') return `/theme-four-experience/textures/gallery/css3logo_painted.webp`;
            return `/theme-four-experience/textures/gallery/${name}_painted.webp`;
        });
    }, [canHover]);
    ...
    // --- GEOMETRY & MATERIALS ---
    const floorTexture = useTexture('/theme-four-experience/textures/gallery/floor.webp');
    const railingTexture = useTexture('/theme-four-experience/textures/gallery/railing.webp');
    const housesTexture = useTexture('/theme-four-experience/textures/gallery/domki.webp');
    const cityTexture = useTexture('/theme-four-experience/textures/gallery/miastotlo.webp');
    const birdTexture = useTexture('/theme-four-experience/textures/gallery/bird_gray.webp');
    const clothespinTexture = useTexture('/theme-four-experience/textures/gallery/klamerka.webp');
    ...
    const materials = useMemo(() => {
        ...
        const thresholdMat = new THREE.MeshBasicMaterial({
            color: '#e0e0e0',
            map: (() => {
                const t = new THREE.TextureLoader().load('/theme-four-experience/textures/corridor/texturadoprogow.webp');
    ...
    return (
        <group ref={groupRef}>
            {!isWarmup && (
                <PositionalAudio
                    ref={audioRef}
                    url="/theme-four-experience/sounds/szummiasta.mp3"
        ...
        >
            ...
            >
                        ...
                        onClick={(e) => {
                            if (isSelected && !isTransitioning) {
                                ...
                                window.parent.postMessage(
                                    { type: EXPERIENCE_MESSAGES.navigate, href: project.href },
                ...
                */}
                ...
                <PositionalAudio
                    ref={paperAudioRef}
                    url="/theme-four-experience/sounds/papersound.mp3"
                    distanceModel="exponential"
