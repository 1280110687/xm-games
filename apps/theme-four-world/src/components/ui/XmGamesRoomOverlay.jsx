...
import { EXPERIENCE_MESSAGES, isContextMessage } from '@xm-games/experience-bridge';
...
// Message names and validation are shared with the Next.js host.
...
const XmGamesRoomOverlay = () => {
    ...
    useEffect(() => {
        const handleMessage = (event) => {
            if (isContextMessage(event, window.location.origin, window.parent)) setContext(event.data.payload);
        };

        window.addEventListener('message', handleMessage);
        window.parent.postMessage({ type: EXPERIENCE_MESSAGES.ready }, window.location.origin);
    ...
    const navigate = (href) => {
        window.parent.postMessage(
            { type: EXPERIENCE_MESSAGES.navigate, href },
